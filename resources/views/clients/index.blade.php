                ...
                <div class="table-responsive">
                    <table class="table table-striped">
                        ...
                        @foreach($clients as $client)
                            <tr>
                                <td>@if ($client->number_of_vencs())
                                    {{$client->name}} <span class="badge badge-danger badge-pill">{{$client->releases->count()}}</span>
                                @else
                                {{$client->name}}
                                @endif
                                </td>
                                <td> @if ($client->releases->count())
                                    <a href="{{route('client.releases', $client->id)}}" class="btn btn-danger btn-icon-style-4 btn-sm"><i class="far fa-arrow-alt-circle-down"></i></a>
                                @else
